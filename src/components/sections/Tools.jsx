...
const Tools = () => {
  return (
    <section id="tools" className="py-20 min-h-screen bg-gradient-to-br from-[#f7f7fb] to-[#eaeaea] dark:from-[#181824] dark:to-[#23272f]">
      <div className="container mx-auto px-2 sm:px-4 max-w-full lg:max-w-5xl">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {tools.map((group, idx) => (
            <div
              key={idx}
              className="rounded-3xl bg-gradient-to-tr from-white/90 to-[#eaeaea]/80 dark:from-[#23272f]/90 dark:to-[#181824]/80 shadow-2xl p-6 sm:p-8 border border-gray-200 dark:border-[#2C3E50]/30 flex flex-col gap-4 items-start hover:scale-[1.03] hover:shadow-2xl transition-all duration-300 group"
            >
              <div className="flex items-center mb-2">
                {group.icon}
                <h3 className="text-lg font-bold text-[#2C3E50] dark:text-[#bdc3c7] tracking-wide text-left">
                  {group.category}
                </h3>
              </div>
              <ul className="space-y-2 mt-2 w-full">
                {group.items.map((item, i) => (
                  <li
                    key={i}
                    className="text-gray-700 dark:text-gray-300 text-base pl-2 relative before:content-[''] before:mr-2 before:font-bold group-hover:text-[#2C3E50] dark:group-hover:text-[#bdc3c7] transition-colors text-left"
                  >
